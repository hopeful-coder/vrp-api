...
# Solve VRP Problem
# @param locationslist List of location objects with lat, lng, name
# @param numVehiclesint Number of vehicles
# @param depotIndexint Index of depot location
# @post solve-vrp
function(locations, numVehicles, depotIndex = 0) {
  ...
  calc_distance - function(lat1, lon1, lat2, lon2) {
    ...
    a - sin(delta_lat2)^2 + 
         cos(lat1_rad)  cos(lat2_rad)  sin(delta_lon2)^2
  ...
  solve_vrp - function(dist_mat, depot, n_vehicles) {
    unvisited - setdiff(1nrow(dist_mat), depot)
    routes - list()
    
    for (v in 1n_vehicles) {
      ...
      for (i in 1customers_per_vehicle) {
        if (length(unvisited) == 0) break
